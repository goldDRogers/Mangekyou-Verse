...
import { getAnimeDetails, getAnimeEpisodes, searchAnime } from '../services/animeService';
import Loader from '../components/Loader';
import ErrorHandler from '../components/ErrorHandler';

const Watch: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  // We need to extend the Anime type or cast appropriately because the detail view response might be different
  ...
  const fetchAnime = async () => {
    ...
    try {
      ...
      const related = await searchAnime(details.title.split(' ')[0]); // Simple related search

      setAnime({
        ...details,
        thumbnail: details.poster || 'https://via.placeholder.com/200x300',
        episodes: details.episodes?.length || 12,
      ...
      setRelatedAnime(mapAnime(related));

    } catch (error) {
      console.error("Error fetching details", error);
      setError("Failed to synchronize with the main server.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return <Loader message="Syncing Rinnegan Node..." />;
  }

  if (error) {
    return <ErrorHandler message={error} retry={fetchAnime} />;
  }
